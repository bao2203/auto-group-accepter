...
interface CatalogItem {
  ...
  itemType: string;
  name: string;
  price?: number; //WARNING: this may be an undefined if the item goes off-sale, defaulting to 0 instead
  lowestPrice?: number; // for limited items
}
...
export async function getCatalogItemPrices(itemIds: number[]): Promise<number> {
...
//get user's RAP
export async function getUserRAP(userId: number) {
    let total = 0;
    let cursor = "";
        ...
        do {
            const _resquest = limiter.wrap(async () => {
                return await client.get(
                    `${INVENTORY}/v1/users/${userId}/assets/collectibles?limit=100&cursor=${cursor}`
                );
            });
            const res = await _resquest();
...
export async function getCurrentAvatarItems(userId: number) {
    try {
        const _resquest = limiter.wrap(async () => {
...
//decline join req
export async function declineJoinRequest(groupId: number, userId: number, csrfToken: string) {
...
// accept join req
export async function acceptRequest(groupId: number, userId: number, csrfToken: string) {
